...
2. Run `./scripts/setup.sh` to fetch dependencies, apply Axiom's macOS Retina
   resize compatibility patch, and compile Raylib.
...
- For rendering changes, include before/after screenshots and run the built-in
  `--qa-motion`, `--qa-resize`, `--qa-first-maximize`,
  `--qa-window-cycle`, and `--qa-perf` modes.
