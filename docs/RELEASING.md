...
1. Update `docs/CHANGELOG.md` and the version/build values in
   `packaging/Info.plist`.
2. Run `lake exe axiom_tests`.
3. Run `./scripts/package.sh` and every relevant `--qa-*` mode documented in README.
4. Run `./scripts/release.sh` to create the app archive and SHA-256 checksum in `dist/`.
